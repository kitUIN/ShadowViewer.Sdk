...
public class ResponderHelper
{
    ...
    /// <summary>
    /// 获取启用的指定Id的INavigationResponder
    /// </summary>
    public static INavigationResponder? GetEnabledNavigateResponder(string id)
    {
        var realId = "ShadowViewer.Plugin." + id;
        return GetEnabledResponder<INavigationResponder>(realId);
    }
    ...
    public static TResponder? GetEnabledResponder<TResponder>(string id) where TResponder : IResponder
    {
        ...
        if (GetResponders<TResponder>()
                    .FirstOrDefault(x => string.Equals(id, x.Id, StringComparison.CurrentCultureIgnoreCase)) is
                { } responder &&
            pluginLoader.IsEnabled(responder.Id) == true) return responder;
        return default;
    }
